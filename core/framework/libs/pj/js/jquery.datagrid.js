...
(function ($, undefined) {
	...
	function Datagrid() {
		...
		this.messages = {
			...
			delete_order: "Are you sure you want to delete the Order ID: ",
			cancel_title: "Cancel confirmation",
			cancel_text: "Are you sure you want to cancel Order ID: ",
			...
			ispaid_title: "Is Paid?",
			ispaid_text: "Is payment made for the products?"
		};
	}
	...
	Datagrid.formatDate = function (date, format) {
	    ...
	    if (pjGrid !== undefined && pjGrid.monthNames !== undefined) {
	    	_monthNames = pjGrid.monthNames;
	    }
	    if (pjGrid !== undefined && pjGrid.dayNames !== undefined) {
	    	_dayNames = pjGrid.dayNames;
	    }
	...
	Datagrid.prototype = {
		_attachDatagrid: function (target, settings) {
			...
			gotoPageAfterUpdate.call(null, 2);

			function gotoPageAfterUpdate (page) {
			...
			.on("click.dg", ".pj-table-icon-delete", function (e) {
				...
				var $this = $(this),
					$tr = $this.closest("tr");
				var recordID = "<strong>"+$tr.find("td:nth-child(2) span").text()+"</strong>";
				var messageText = '';
				var model = $this.data('model').toLowerCase();
				switch(model) {
					case 'order':
						messageText = self.messages.delete_order+recordID+" ?";
					break;
					default:
						messageText = self.messages.delete_text;
				}
				swal({
	            ...
	            case "OK":
	            	if (model == 'order') {
	            		var origin = data.origin.toLowerCase();
	              	var $listObj = $('#pending-'+origin);
									$listObj.html(' - '+data.count);
	            	}
			...
			.on("click.dg", ".pj-table-icon-cancel", function (e) {
				if (e && e.preventDefault) {
					e.preventDefault();
				}
				var $this = $(this),
				...
				var recordID = "<strong>"+$tr.find("td:nth-child(2) span").text()+"</strong>";
				swal({
					title: self.messages.cancel_title,
					text: self.messages.cancel_text +recordID+" ?",
					html: true,
					type: "warning",
					showCancelButton: true,
					confirmButtonColor: "#C1C1C1",
					confirmButtonText: self.messages.btn_yes,
					cancelButtonText: self.messages.btn_no,
					closeOnConfirm: false,
					showLoaderOnConfirm: true,
					customClass: 'cancel_popup'
				}, function () {
					$.post($this.attr("href")).done(function (data) {
					    if (!(data && data.status)) {
							swal("Error!", '', "error");
			...
			.on("click.dg", ".pj-table-icon-menu", function (e) {
				...
				$.post(url, {
					...
					self._loadDatagrid.call(self, target, inst.settings.dataUrl, inst.settings.content.column, inst.settings.content.direction, inst.settings.content.page, inst.settings.content.rowCount);
					if (data && data.status && data.status !== "OK" && data.text) {
                        swal('Error', data.text, 'error');
                    }
				});
		...
		_loadDatagrid: function (target, url, column, direction, page, rowCount) {
			...
			$.get(url, $.extend(obj, inst.settings.cache)).done(function (data) {
				inst.settings.content = data;
				$.data(target, PROP_NAME, inst);
				self._renderDatagrid.call(self, target, page);
			});
		},
		_renderDatagrid: function (target) {
			...
			for (i = 0, iCnt = inst.settings.columns.length; i < iCnt; i++) {
				$th = $("<th>");
				var $ordersTable = $("#orderIndexTitle");
			...
			if (inst.settings.content.data && inst.settings.content.data.length === 0) {
				...
				$td.addClass("pj-table-norecords");
				$td.appendTo($tr);
				$tr.appendTo($tbody);
			}
			if (inst.settings.content.data) {
				//console.log("For page no:" + inst.settings.content.page);
            ...
            .attr("data-type", inst.settings.content.data[i].type);
					...
					for (j = 0, jCnt = inst.settings.fields.length; j < jCnt; j++) {
						...
						// MEGAMIND
						if(inst.settings.content.data[i].expected_delivery) {
							if (inst.settings.columns[j].text == "ED") {
							    if (inst.settings.content.data[i].delivered_customer == 1) {
                                
								    if (inst.settings.content.data[i].delivered_time < inst.settings.content.data[i].expected_delivery) {
								
								        $td.addClass("delivery-success");
									} else {
									  	$td.addClass("delivery-failed");
									}
								}
							}
						}
						...
						switch (inst.settings.columns[j].type) {
							case "text":
								if ($("#posWrapper").length > 0) {
                                    if (inst.settings.columns[j].text == 'Delivery Time') {
										if (inst.settings.content.data[i].deliver_sts == 'success') {
											$td.addClass("bg-success");
										} else {
                                            if (inst.settings.content.data[i].deliver_sts == 'failure') {
							...
							case "toggle":
								...
								if (inst.settings.columns[j].renderer === undefined) {
									...
									case inst.settings.content.data[i][inst.settings.fields[j]] == inst.settings.columns[j].positiveValue:
									    if ($ordersTable.text() == "Order List") {
									    	$span.html(['<div role="button" tabindex="0" aria-disabled="true" class="', (inst.settings.columns[j].positiveClass ? inst.settings.columns[j].positiveClass : 'btn btn-primary btn-xs no-margins'), (isDisabled ? ' disabled' : ''), '"><i class="', (inst.settings.columns[j].positiveIcon ? inst.settings.columns[j].positiveIcon : 'fa fa-check'), '"></i> ', inst.settings.columns[j].positiveLabel, '</div>'].join(''));
									    } else {
									    	$span.html(['<div role="button" tabindex="0" aria-disabled="', (isDisabled ? 'true' : 'false'), '" class="', (inst.settings.columns[j].positiveClass ? inst.settings.columns[j].positiveClass : 'btn btn-primary btn-xs no-margins'), (isDisabled ? ' disabled' : ''), '"><i class="', (inst.settings.columns[j].positiveIcon ? inst.settings.columns[j].positiveIcon : 'fa fa-check'), '"></i> ', inst.settings.columns[j].positiveLabel, '</div>'].join(''));
									    }
									...
									case inst.settings.content.data[i][inst.settings.fields[j]] == inst.settings.columns[j].negativeValue:
									    ...
									    if ($ordersTable.text() == "Order List") {
									    	if ([inst.settings.fields[j]][0] == "order_despatched") {
									    		if (inst.settings.content.data[i].status == "cancelled") {
                                                   $span.html(['<div role="button" tabindex="0" aria-disabled="true" class="', (inst.settings.columns[j].negativeClass ? inst.settings.columns[j].negativeClass : 'btn btn-negative btn-xs no-margins'), (isDisabled ? ' disabled' : ''), '"><i class="', (inst.settings.columns[j].negativeIcon ? inst.settings.columns[j].negativeIcon : 'fa fa-times'), '"></i> ', inst.settings.columns[j].negativeLabel, '</div>'].join(''));
									    		} else {
									    		$span.html(['<div role="button" tabindex="0" aria-disabled="', (isDisabled ? 'true' : 'false'), '" class="', (inst.settings.columns[j].negativeClass ? inst.settings.columns[j].negativeClass : 'btn btn-negative btn-xs no-margins'), (isDisabled ? ' disabled' : ''), '"><i class="', (inst.settings.columns[j].negativeIcon ? inst.settings.columns[j].negativeIcon : 'fa fa-times'), '"></i> ', inst.settings.columns[j].negativeLabel, '</div>'].join(''));
									    		var len = inst.settings.content.data[i].expected_delivery.split(" ");
									    		var d = new Date();
										    
										        var dateToday = Datagrid.formatDate(d,'dd-MM-yyyy');
									    		
									    		if (len.length == 2) {
		                                            
		                                            var dateItems1 = len[0].split("-");
		                                            var dateItems2 = dateToday.split("-");
									    			//if (len[0] > dateToday) {
									    			if (dateItems1[0] > dateItems2[0] || dateItems1[1] > dateItems2[1] || dateItems1[2] > dateItems2[2]) {

									    				//console.log("Today is not a delivery date");
										    			$span.on("click.dg", ".btn-negative", function (e) {
										    				swal({
									            	...
									            	$span.on("click.dg", ".btn-negative", function (e) {
									            		var $this = $(this),
															$tr = $this.closest("tr"),
															$id = $tr.attr("data-id").split("_");
									            	
														$.post("index.php?controller=pjAdminPosOrders&action=pjActionSaveOrderDespatched&id="+$id[1]).done(function (data) {
														    if (!(data && data.status)) {
																swal("Error!", '', "error");
															}
															switch (data.status) {
									    	...
									    	if ([inst.settings.fields[j]][0] == "delivered_customer") { 
									    		if (inst.settings.content.data[i].order_despatched == 1) { 
                                                    $span.html(['<div role="button" tabindex="0" aria-disabled="false" class="', (inst.settings.columns[j].negativeClass ? inst.settings.columns[j].negativeClass : 'btn btn-negative btn-xs no-margins'), (isDisabled ? ' disabled' : ''), '"><i class="', (inst.settings.columns[j].negativeIcon ? inst.settings.columns[j].negativeIcon : 'fa fa-times'), '"></i> ', inst.settings.columns[j].negativeLabel, '</div>'].join(''));
									    		    inst.settings.columns[j].editable = true;
									    		    if (inst.settings.content.data[i].is_paid == 0) {
											    		...
											    		$span.on("click.dg", ".btn-negative", function (e) {
															...
															swal({
																title: self.messages.ispaid_title,
																text: self.messages.ispaid_text,
																showCancelButton: true,
																confirmButtonText: self.messages.btn_yes,
																cancelButtonText: self.messages.btn_no,
																closeOnConfirm: false,
																...
																if (data) {
																	$.post("index.php?controller=pjAdminPosOrders&action=pjActionSaveOrderPaid&id="+$id[1]).done(function (data) {
																	    if (!(data && data.status)) {
																			swal("Error!", '', "error");
																		}
																		switch (data.status) {
																	...
																	$.post("index.php?controller=pjAdminPosOrders&action=pjActionSaveDeliveredCustomer&id="+$id[1]).done(function (data) {
																	    if (!(data && data.status)) {
																			swal("Error!", '', "error");
																		}
																		switch (data.status) {
										    			...
										    			$span.on("click.dg",function(){
											    			var $data_id = $(this).parents("tr").attr("data-id").split("_");
											    		
													       $.post("index.php?controller=pjAdminOrders&action=pjActionSaveDeliveredCustomer&id="+$data_id[1]).done(function (data) {
															    if (!(data && data.status)) {
																	swal("Error!", '', "error");
																}
																switch (data.status) {
									    		...
									    		} else if(inst.settings.content.data[i].delivered_customer == 1) {
									    	    } else {
									    	    //console.log("hey");
									    			$span.html(['<div role="button" tabindex="0" aria-disabled="true" class="', (inst.settings.columns[j].negativeClass ? inst.settings.columns[j].negativeClass : 'btn btn-negative btn-xs no-margins'), (isDisabled ? ' disabled' : ''), '"><i class="', (inst.settings.columns[j].negativeIcon ? inst.settings.columns[j].negativeIcon : 'fa fa-times'), '"></i> ', inst.settings.columns[j].negativeLabel, '</div>'].join(''));
									    		}
									    	} else if ([inst.settings.fields[j]][0] == "is_paid") {
									    		    if (inst.settings.content.data[i].status == "cancelled") {
									    		    	$span.html(['<div role="button" tabindex="0" aria-disabled="true" class="', (inst.settings.columns[j].negativeClass ? inst.settings.columns[j].negativeClass : 'btn btn-negative btn-xs no-margins'), (isDisabled ? ' disabled' : ''), '"><i class="', (inst.settings.columns[j].negativeIcon ? inst.settings.columns[j].negativeIcon : 'fa fa-times'), '"></i> ', inst.settings.columns[j].negativeLabel, '</div>'].join(''));
									    		    } else {
									    		    	$span.html(['<div role="button" tabindex="0" aria-disabled="', (isDisabled ? 'true' : 'false'), '" class="', (inst.settings.columns[j].negativeClass ? inst.settings.columns[j].negativeClass : 'btn btn-negative btn-xs no-margins'), (isDisabled ? ' disabled' : ''), '"><i class="', (inst.settings.columns[j].negativeIcon ? inst.settings.columns[j].negativeIcon : 'fa fa-times'), '"></i> ', inst.settings.columns[j].negativeLabel, '</div>'].join(''));
					...
					if (bCnt > 0) {
						...
						for (j = 0; j < bCnt; j++) {
							...
							if ($ordersTable.text() == "Order List") {
								if (inst.settings.content.data[i].deleted_order == 0) {
								// var gridType = $("#posWrapper #grid-type").val();
								//if (inst.settings.content.data[i].order_despatched == 0) {
							    	
							    //} else {
							    	//$a.removeAttr("href");
							    //}
								// console.log("Grid type: "+gridType);
								//if(inst.settings.content.data[i].origin == "Pos" && inst.settings.content.data[i].status == "delivered") {
									if(inst.settings.content.data[i].status == "delivered") {
	      						$a.removeClass("btn-primary btn-outline btn-sm m-l-xs pj-table-icon-" + inst.settings.buttons[j].type);
									  $a.html("");
									  appendButton = false;
									} else {
								...
								$a.addClass("btn-primary btn-outline btn-sm m-l-xs pj-table-icon-" + inst.settings.buttons[j].type);
								$("<i>").addClass("fa fa-pencil").appendTo($a);
								if($("#productIndexTitle").length > 0) { //To add pagination to products list
									var category = $("#filter_category").val();
									var old_href = $a.attr('href');
									var new_href = old_href+"&category="+category+"&page="+inst.settings.content.page;
							...
							case "delete":
							    if ($ordersTable.text() == "Order List") {
							    	if (inst.settings.content.data[i].deleted_order == 0) {
											...
											if ($role != ADMINID) {
												$a.removeClass("btn btn-danger btn-outline btn-sm m-l-xs pj-table-icon-delete");
										    $a.html("");
										   	appendButton = false;
											}
									} else {
								...
								if (typeof(inst.settings.buttons[j].model) !== 'undefined') {
								 	$a.attr("data-model", inst.settings.buttons[j].model);
								} else {
									$a.attr("data-model", "common");
								}
							break;
							case "cancel":
							  //   if ($ordersTable.text() == "Order List") {
								...
								// }
									...
									if (inst.settings.content.data[i].status == "pending" && inst.settings.content.data[i].deleted_order == 0) {
										$a.addClass("btn-danger btn-outline btn-sm m-l-xs pj-table-icon-cancel");
										$("<i>").addClass("fa fa-ban").appendTo($a);
									} else {
										appendButton = false;
									}
								break;
							case "deleteGDPR":
								$a.addClass("btn-secondary btn-outline btn-sm m-l-xs pj-table-icon-delete");
								$("<i>").addClass("fa fa-trash").appendTo($a);
								$a.text("GDPR");
							break;
					    case "delay":
				        if (inst.settings.content.data[i].delivered_customer == 0) {
					        $a.addClass("btn-primary btn-outline btn-sm m-l-xs");
									$a.attr("data-name", inst.settings.content.data[i].surname);
									$a.attr("data-phone", inst.settings.content.data[i].phone_no);
					        $a.text("SMS");
									$a.click(function () {
										var order_id  = parseInt($(this).parents("tr").data("id").split("_")[1]);
										var data_phone = $(this).attr("data-phone");
						        var data_name = $(this).attr("data-name");
										$.ajax({ 
							        type: 'POST',
							        async: false,
							        url: 'index.php?controller=pjAdminPosOrders&action=pjActionGetReasonList',
							        data: {
							        	id: order_id
							        },
							        success: function (data) {
							        	//console.log(data);
							        	$("#delay_container").html(data);
							        	$("#message").val('');
							        	$("#smsModal").modal("show");
						            var submitBtn = $("#d_msg");
						            //submitBtn.prop('disabled', false);
						            //var $data_id = $(this).parents("tr").attr("data-id").split("_");
						            $("#delay_cname").text(data_name);
						            $("#delay_cphone").text(data_phone);
						            submitBtn.click(function(e){
						            	$(this).prop('disabled', true);
										    	$.ajax({ 
										        type: 'POST',
										        async: false,
										        url: 'index.php?controller=pjAdminPosOrders&action=pjActionDelayMessage',
										        data: {
										        	id: function() {
										        		return order_id;
										        	},
							...
							break;
							case "info":
								$a.addClass("btn-outline btn-sm m-l-xs pj-table-icon-" + inst.settings.buttons[j].type);
								if(inst.settings.content.data[i].delivered_time != '-') {
									if(inst.settings.content.data[i].deliver_sts == "failure") {
								...
								if ($ordersTable.text() == "Order List") { 
									$a.click(function() {
										var $id = parseInt($(this).parents("tr").data("id").split("_")[1]);
										$.get("index.php?controller=pjAdminPosOrders&action=pjActionGetSingleOrderInfo&id="+$id).done(function (data) {
											swal({
												html:true,
												title: "",
												text: data,
							...
							break;
              case "OrderInfo":
                if(inst.settings.content.data[i].type == "AR") {
                  appendButton = false;
                  break;
                }
							case "CancelInfo":
								...
								$("<i>").addClass("fa fa-info").appendTo($a);
								$a.click(function(type) {
                  var $id = parseInt($(this).parents("tr").data("id").split("_")[1]);
									var dataType = $(this).parents("tr").data("type");
                  var action = ""
                  if(dataType == "RO") {
                    action = "pjActionGetCancelOrderInfo";
                  } else {
                    action = "pjActionGetAdminReturnOrderInfo";
                  }
									var type = $(this).text().trim();
									$.get("index.php?controller=pjAdminReports&action="+action+"&id="+$id+"&type="+type).done(function (data) {
										swal({
											html:true,
											title: "",
											text: data,
                      ...
                      customClass: "swal-width",
							...
							case "sendSMS":
				        $a.addClass("btn-primary btn-outline btn-sm m-l-xs");
								if(inst.settings.content.data[i].user_type != "Guest") {
									$a.attr("data-name", inst.settings.content.data[i].user_type);
								} 
								if(inst.settings.content.data[i].user_type == "Guest") {
									$a.attr("data-name", inst.settings.content.data[i].guest_un);
								}
								$a.attr("data-phone", inst.settings.content.data[i].phone);
						        $a.text("sendSMS");

								$a.click(function () {
			            $("#VoucherPopup").modal("show");
			            var submitBtn = $("#tq_msg_submit");
			            var $data_name = $(this).attr("data-name");
			            var $data_phone = $(this).attr("data-phone");
									$("#review_cname").text($data_name);
									$("#review_cphone").text($data_phone);
						            submitBtn.click(function(e){
							    $.ajax({
						        type: 'POST',
						        async: false,
						        url: 'index.php?controller=pjAdminReviews&action=pjActionVoucherMessage',
						        data: {
						        	phone: function() {
					        			return $data_phone;
						        	},
						        	tq_msg: function() {
					        			return $("#tq_message").val();
						        	}
						        },
						        success: function (data) {
						        	},
							...
							default:
							    if ($ordersTable.text() == "Order List") {
								    //if (inst.settings.content.data[i].order_despatched == 0) {
										...
										// var url = $a.attr('href')+'&origin='+inst.settings.content.data[i].origin;
										// console.log(inst.settings.content.data[i].origin);
										// $a.attr('href',url);
										$("<i>").addClass("fa fa-" + inst.settings.buttons[j].type).appendTo($a);
							        //} else {
										//$a.removeAttr("href");
							...
							if (inst.settings.buttons[j].type == "menu" && inst.settings.buttons[j].items !== undefined) {
								...
								$button = $("<button>").attr("data-toggle", "dropdown").addClass("btn btn-primary btn-outline btn-sm dropdown-toggle").appendTo($group);
			...
			if (inst.settings.select !== FALSE && $.fn.iCheck !== undefined) {
				...
				$table.find(".pj-table-toggle-rows").iCheck({
					...
					if ($this.is(":checked")) {
						$checkboxes.iCheck("check").closest("tr").addClass("pj-table-row-active");
					} else {
						$checkboxes.iCheck("uncheck").closest("tr").removeClass("pj-table-row-active");
					}
				});
		...
		bindSortable: function (target) {
			...
			$target.find(".table-sortable tbody").sortable({
				cursor: "move",
				helper: fixHelper,
